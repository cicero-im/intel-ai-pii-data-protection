...
class PII_Data_Detagger:
    ...
    def __generate_alphanum(self):
        return self.alphanum.sub(
            lambda x: str(np.random.choice(self.anlist)), self.format
        )
    ...
    def obfuscate_string(self, val, slen):
        ...
        """
        return "".join(
            np.random.choice(list(string.ascii_letters + "  "), slen)
        ).title()
    ...
    def detag_pii_data(self, input_text):
        ...
        for entity in ner_results:
            ...
            if entity["entity_group"] in ["PER", "ORG", "LOC"]:
                ...
                input_text = (
                    input_text[: entity["start"]]
                    + random_string
                    + input_text[entity["end"] :]
                )
        return input_text
